...
const Sprites = {
    ...
    preload() {
        return this.ready = new Promise(async (resolve) => {
            // In the Vite build the bundled graphics live in the public/
            // directory and are served from the site root.
            let [bin, json] = await Promise.all([
                fetch(new URL("gfx.bin", import.meta.env.BASE_URL || "/").toString()),
                fetch(new URL("gfx.json", import.meta.env.BASE_URL || "/").toString())
            ]);
            ...
            let step = () => {
                ...
                if (finished < json.length) {
                    if (window.loader) window.loader.innerHTML = Math.floor(100 * finished / json.length) + "%";
                    setTimeout(step, 0);
                } else {
                    if (window.loader) window.loader.remove();
                    resolve();
                }
            };
            setTimeout(step, 0);
