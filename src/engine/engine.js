...
class Engine {
    constructor(viewer, definition) {
        ...
        for (let i = 1; i < this.players.length; i++) {
            const def = this.definition.players[i];
            if (def && def.is_cpu) {
    ...
    startLoop() {
        // requestAnimationFrame-based loop with frame limiting.
        // Falls back to setInterval if rAF is unavailable (tests, SSR).
        const frameTime = 1000 / this.frameRate;
        if (typeof window === 'undefined' || typeof window.requestAnimationFrame !== 'function') {
            this.loop = setInterval(this.processLoop.bind(this), frameTime);
            return;
        }
        ...
        const tick = (timestamp) => {
            ...
            if (delta >= frameTime) {
                this._lastTickTime = timestamp - (delta % frameTime);
                this.processLoop();
            }
    ...
    stopLoop() {
        this._stopped = true;
        if (typeof this.loop === 'number' && typeof window !== 'undefined' && typeof window.cancelAnimationFrame === 'function') {
    ...
    processAI() {
        if (!this.aiPlayers || this.aiPlayers.length === 0) return;
        for (const ai of this.aiPlayers) {
            try { ai.tick(this.framesCount); } catch (err) { /* AI errors are non-fatal */ }
        }
    }
    ...
    addUnit(unit) {
        ...
        // Fire a creation sound only for the human player's units to avoid
        // spamming the output during AI-heavy phases.
        if (unit.player === this.current_player) {
            audioManager.play('villager_select', { volume: 0.25 });
        }
    }
    addBuilding(building) {
        ...
        if (building.player === this.current_player) {
            audioManager.play('building_complete', { volume: 0.4 });
        }
    }
